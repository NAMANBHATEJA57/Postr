...
const EnvelopeAnimation = dynamic(() => import("@/components/envelope/EnvelopeAnimation"), { ssr: false });
...
interface ViewPublicClientProps {
  postcardId: string;
...
export default function ViewPublicClient({ postcardId, initialData, status }: ViewPublicClientProps) {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      setPhase(isCreator ? "reveal" : "envelope");
    }
  }, [initialData, isCreator, status]);

  useEffect(() => {
    setShareUrl(typeof window !== "undefined" ? `${window.location.origin}/public/post/${postcardId}` : "");
  ...
  if (phase === "envelope" && postcard) {
    return <EnvelopeAnimation toName="The World" fromName={postcard.fromName} onOpen={() => setPhase("reveal")} />;
  }
    ...
    <div className="min-h-dvh bg-[#f8f4ef] flex flex-col items-center pb-[100px]">
      ...
      <nav className="w-full flex h-[80px] items-center justify-between px-6 md:px-10 max-w-[1200px] mx-auto">
        ...
        <div className="hidden md:flex items-center gap-[32px] text-[#4a4a4a] text-[14px] font-sans font-medium">
          ...
          <button 
            onClick={() => copyLink()}
            className="flex items-center gap-[8px] hover:text-[#1a1a1a] transition-colors animate-none"
          >
            {copied ? (
              ...
              <>
                Share postcard <span className="material-symbols-rounded text-[18px]">link</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="md:hidden flex flex-wrap items-center justify-center gap-4 py-4 px-6 w-full text-[#4a4a4a] text-[13px] font-sans font-medium border-b border-[#eadfd5]/50">
        ...
        <button 
          onClick={() => copyLink()}
          className="flex items-center gap-[6px] text-[#4a4a4a] animate-none"
        >
          {copied ? (
            <span className="text-[#8b7364] flex items-center gap-1 font-sans text-[13px] font-medium">
      ...
      <div className="mx-auto flex w-full max-w-[880px] flex-col items-center px-4 pt-[40px] md:pt-[60px]">
        ...
        <div className="text-center mb-[40px] flex flex-col items-center">
          <h1 className="font-serif font-bold text-[48px] md:text-[56px] text-[#1a1a1a] tracking-tight leading-[1.1] mb-[12px]">
            {isCreator ? "your postcard is ready." : `a postcard\nfrom ${postcard.fromName.toLowerCase()}.`}
          </h1>
        ...
        {/* COPY LINK WIDGET */}
        <div className="mt-[32px] w-full max-w-[480px]">
          <div className="flex items-center justify-between rounded-[8px] border border-[#eadfd5] bg-white px-4 py-3 shadow-[0_4px_12px_rgba(88,61,44,0.03)]">
            <span className="truncate pr-4 font-sans text-[13px] text-[#4c433d]">{shareUrl}</span>
            <button type="button" onClick={copyLink} className="flex items-center gap-1.5 font-sans text-[13px] text-[#1a1a1a] transition-all hover:opacity-80 animate-none">
              {copied ? (
                <span className="text-[#8b7364] flex items-center gap-1 font-sans text-[12px] font-medium tracking-tight">
                  <span className="material-symbols-rounded text-[16px]">check</span>
                  copied
                </span>
              ) : (
        ...
        {/* FOOTER CTA BUTTONS */}
        <div className="mt-[48px] flex flex-col sm:flex-row items-center justify-center gap-[16px] w-full max-w-[480px]">
          <Link 
            href="/public/create" 
            className="w-full sm:flex-1 px-[24px] py-[16px] rounded-[8px] border border-[#1a1a1a] bg-transparent font-sans text-[14px] text-[#1a1a1a] transition hover:bg-black/5 text-center font-medium"
          >
            Share a postcard
          </Link>
          <Link 
