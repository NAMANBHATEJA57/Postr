...
function CreatePrivatePageInner() {
  const router = useRouter();
  const params = useParams();

  const spaceId = typeof params.spaceId === "string" ? params.spaceId.toUpperCase() : "";
  ...
  const canPublish = useMemo(() => {
    const hasMoment = Boolean(mediaFile || title.trim() || message.trim());
    const hasNames = Boolean(toName.trim() && fromName.trim());
    return hasMoment && hasNames && Boolean(spaceId);
  }, [fromName, mediaFile, message, spaceId, title, toName]);
  ...
  const handlePublish = useCallback(async () => {
    ...
    if (typeof window !== "undefined") {
      ...
      let spaceData = { id: spaceId, createdAt: new Date().toISOString(), postcards: [] as ApiPostcardResponse[] };
      if (cached) {
        try {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        if (cached) {
          try {
            const spaceData = JSON.parse(cached);
            if (spaceData && Array.isArray(spaceData.postcards)) {
              spaceData.postcards = spaceData.postcards.map((card: any) => {
  ...
  }, [canPublish, fromName, mediaFile, message, password, router, spaceId, stampId, submitting, title, toName]);
    ...
    <main className="min-h-dvh bg-[#f8f4ef] flex flex-col items-center">
      ...
      <div className="w-full max-w-[480px] flex flex-col pt-[40px] pb-[100px] px-4">
        ...
        <div className="flex flex-col gap-[32px]">
          ...
          {/* Who is this for? */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-sans font-medium text-[14px] text-[#1a1a1a]">Who is this for?</label>
            <input
              type="text"
              value={toName}
        ...
        <div className="mt-[48px] flex flex-col items-center gap-[12px]">
          ...
          <button
            ...
            className="w-full bg-[#1a1a1a] text-white font-sans text-[15px] py-[16px] rounded-[8px] transition hover:bg-black disabled:opacity-50 shadow-md"
          >
            {isSuccess ? "Moment sent" : uploading ? "Uploading media..." : submitting ? "Sending..." : "create a postcard"}
          </button>

          <p className="font-sans text-[12px] text-[#888888]">
            this postcard will go to your private space
          </p>
        </div>
      </div>
    </main>
